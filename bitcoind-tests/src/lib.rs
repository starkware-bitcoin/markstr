...
impl TestNode {
    ...
    fn start_with_funded_wallet() -> Result<Self> {
        ...
        let (wallet_client, wallet_name) =
            Self::ensure_wallet_and_fund(&base_client, base_url, &auth)?;
        Ok(Self {
            rpc: wallet_client,
            _proc: Some(node),
    ...
    fn start_docker_bitcoind() -> Result<DockerBitcoind> {
        ...
        let docker_args = vec![
            ...
            "-d".to_string(),
            "-p".to_string(),
            format!("{}:18443", rpc_port),
            "-v".to_string(),
            format!("{}:/home/bitcoin/.bitcoin", data_dir_path.display()),
